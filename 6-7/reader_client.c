...
void reader_process(int reader_id, const char *ip, int port) {
    ...
    while (1) {
        ...
        if (sock < 0) {
            printf("Reader %d (PID %d): Reconnecting in 2 seconds...\n", reader_id, getpid());
            sleep(2);
            continue;
        }

        printf("Reader %d (PID %d): Connected to server\n", reader_id, getpid());
        ...
        while (1) {
            ...
            printf("Reader %d (PID %d): Requesting index %d\n", reader_id, getpid(), index);
            ...
            if (valread <= 0) {
                ...
                else printf("Reader %d (PID %d): Server disconnected\n", reader_id, getpid());
                break;
            }
            buffer[valread] = '\0';

            int received_index, value;
            if (sscanf(buffer, "VALUE %d %d", &received_index, &value) != 2) {
                printf("Reader %d (PID %d): Failed to parse VALUE: '%s'\n", reader_id, getpid(), buffer);
                continue;
            }
            printf("Reader %d (PID %d): Received: Index %d, Value %d\n", 
                   reader_id, getpid(), received_index, value);
